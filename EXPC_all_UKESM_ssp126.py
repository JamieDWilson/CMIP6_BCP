...
#UKESM for historical+ssp370 and piControl, and their anomaly
# !! the branch of historical run starts from year 400 since 1850 of the piControl
data_dir='/badc/cmip6/data/CMIP6/'
output_dir='/gws/pw/j05/cop26_hackathons/bristol/project03/output_nc/CONTROL/EXP_ssp126/' #'/gws/pw/j05/cop26_hackathons/bristol/project03/output_nc/CONTROL/'
...
#expc
dir_expc = data_dir+Scenario_hist+'/'+centre_name+'/'+model_name+'/'+exp_pi+'/'+real+'/Omon/'+data_expc+'/'+grG+'/latest/'
file_pi_expc=glob.glob(dir_expc+'/*.nc')
expc_list=(file_pi_expc[6:12])
data_pi_expc = xr.open_mfdataset(expc_list)

## READ files for historical run
#expc
dir_expc = data_dir+Scenario_hist+'/'+centre_name+'/'+model_name+'/'+exp_hist+'/'+real+'/Omon/'+data_expc+'/'+grG+'/latest/'
file_hist_expc=glob.glob(dir_expc+'/*.nc')
data_hist_expc = xr.open_mfdataset(file_hist_expc)

## READ files for ssp126 run
#expc
dir_expc = data_dir+Scenario_ssp+'/'+centre_name+'/'+model_name+'/'+exp_ssp+'/'+real+'/Omon/'+data_expc+'/'+grG+'/latest/'
file_ssp_expc=glob.glob(dir_expc+'/*.nc')
...
nend1=time_all.size
# UKESM 1850 is year 2250 so modify timeslice accordingly
datch=-400
