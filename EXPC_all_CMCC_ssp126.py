...
#CMCC-ESM2 expc for historical+ssp126 and piControl, and their anomaly
# !! the branch of historical run starts from year 1850 of the piControl
#!! Only yealry files abailable for CMCC-ESM2 and expc
download_dir='/gws/pw/j05/cop26_hackathons/bristol/project03/input_nc/multimodel/'
data_dir='/badc/cmip6/data/CMIP6/'
output_dir='/gws/pw/j05/cop26_hackathons/bristol/project03/output_nc/CONTROL/EXP_ssp126/'
...
Ar=xr.open_mfdataset('/badc/cmip6/data/CMIP6/CMIP/CMCC/CMCC-ESM2/piControl/r1i1p1f1/Ofx/areacello/gn/latest/areacello_Ofx_CMCC-ESM2_piControl_r1i1p1f1_gn.nc')
## READ files for piControl run
#expc
file_pi_expc=glob.glob(download_dir+'piControl/'+data_expc+'_Oyr_'+model_name+'_'+exp_pi+'_'+real+'_'+grG+'_'+'*.nc')
expc_list=(file_pi_expc[0:2])
data_pi_expc = xr.open_mfdataset(expc_list)

## READ files for historical run
#expc
file_hist_expc=glob.glob(download_dir+'historical/'+data_expc+'_Oyr_'+model_name+'_'+exp_hist+'_'+real+'_'+grG+'_'+'*.nc')
data_hist_expc = xr.open_mfdataset(file_hist_expc)

## READ files for ssp126 run
#expc
file_ssp_expc=glob.glob(download_dir+'ssp126/'+data_expc+'_Oyr_'+model_name+'_'+exp_ssp+'_'+real+'_'+grG+'_'+'*.nc')
...
nstart=0 # CMCC starts at year 1850,at the beginning of file 1850 which is the first file read
nend1=time_all.size
# CMCC 1850 is year 1850 
datch=0
timeslices_pi=[element-datch for element in timeslices]

## Caclulate exp control historical and ssp
...
## merge exp historical and ssp into one timeserie
...
POC_all.to_netcdf(output_dir+'POC_yearly_all_'+model_name+'_ssp126.nc',mode='w')
